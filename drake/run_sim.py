...
# def build_and_simulate_trajectory(q_traj, gripper_actions):
#     """
#     Simulate the robot's motion along the generated trajectory, 
#     with multiple gripper open/close actions.
    
#     Parameters:
#     - q_traj: Trajectory for the robot's motion.
...
#             super().__init__()
#             self.gripper_actions = sorted(gripper_actions, key=lambda x: x[0])  # Sort by time
#             self.DeclareVectorOutputPort("wsg.position", BasicVector(1), self.CalcGripperPosition)
...
#     # Start simulation
#     meshcat.StartRecording(set_visualizations_while_recording=False)
#     max_simulation_time = max([action[0] for action in gripper_actions]) + 1.0
...
def extract_vase_poses():
    ...
    goal_poses = []
    # append target
    goal_poses.append(goal_pose_target) # No. 1
    # above target (in the z-axis)
    goal_poses.append( # No. 2
        RigidTransform(desired_rotation, 
                       np.array([target_translation[0], target_translation[1], 0]) + np.array([0, 0, 0.6])))
    # append translated final pose (in the z-axis)
    goal_poses.append( # No. 3
        RigidTransform(desired_rotation, 
                       np.array([-0.2, 0.4, 0.6]))
    )
    # append final pose
    goal_poses.append( # No. 4
        RigidTransform(desired_rotation, 
                       np.array([goal_poses[-1].translation()[0],
                                 goal_poses[-1].translation()[1],
                                 target_translation[2]]))
    ...
    # prepend initial pose (close to target in the y-axis)
    goal_poses.insert(0, # No. 0
        RigidTransform(desired_rotation, 
                       target_translation + desired_rotation @ np.array([0, -0.15, 0]).T))
    
    # go back to the initial pose
    goal_poses.append(goal_poses[4]) # No. 5
    goal_poses.append(goal_poses[3]) # No. 5
    goal_poses.append(goal_poses[2]) # No. 6
    goal_poses.append(goal_poses[1]) # No. 7
...
if __name__ == "__main__":
    ...
    #     # Define gripper actions based on segment times
    # gripper_close_times = [
    #     segment_times[2][0],  # Close after goalpose No.2
    #     segment_times[4][1]   # Close after goalpose No.4
    # ]
    # gripper_open_times = [
    #     segment_times[-1][1],  # Open after goalpose No.6
    # ]

    # build_and_simulate_trajectory(q_traj_combined,
    #                               gripper_close_time=gripper_close_times, # target goal pose
    #                               gripper_open_time=gripper_open_times) # end goal pose
    ...
    # Simulate the trajectory with gripper actions
    # build_and_simulate_trajectory(q_traj_combined, gripper_actions)
